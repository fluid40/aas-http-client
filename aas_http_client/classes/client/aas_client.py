        ...
        for url in urls:
            ...
            try:
                ...
                if response.status_code not in (STATUS_CODE_200, STATUS_CODE_201, STATUS_CODE_204):
                    error_messages.update({response.status_code: response.reason})
...
def _create_client(config_dict: dict, basic_auth_password: str, o_auth_client_secret: str, bearer_auth_token: str) -> AasHttpClient | None:
    ...
    _logger.debug("Using server configuration:")
    _logger.debug(f"BaseUrl: '{client.base_url}'")
    _logger.debug(f"TimeOut: '{client.time_out}'")
    _logger.debug(f"HttpsProxy: '{client.https_proxy}'")
    _logger.debug(f"HttpProxy: '{client.http_proxy}'")
    _logger.debug(f"ConnectionTimeOut: '{client.connection_time_out}'.")
    _logger.debug(f"SSLVerify: '{client.ssl_verify}'.")
    _logger.debug(f"TrustEnv: '{client.trust_env}'.")
    _logger.debug(f"EncodedIds: '{client.encoded_ids}'.")

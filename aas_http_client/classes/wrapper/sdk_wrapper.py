...
class SdkWrapper:
    ...
    def get_asset_administration_shell_by_id(self, aas_identifier: str) -> model.AssetAdministrationShell | None:
        ...
        if not self._client.shells:
            __logger.error("Shell API is not initialized in the client. Call 'initialize()' method of the client before calling this method.")
            return None

        content = self._client.shells.get_asset_administration_shell_by_id(aas_identifier)

        if not content:
            __logger.warning(f"No shell found with ID '{aas_identifier}' on server.")
            return None
    ...
    def put_asset_administration_shell_by_id(self, aas_identifier: str, aas: model.AssetAdministrationShell) -> bool:
        ...
        if aas_data is None:
            __logger.error(f"Failed to serialize Asset Administration Shell with ID '{aas_identifier}' to dictionary.")
            return False
    ...
    def get_thumbnail_aas_repository(self, aas_identifier: str) -> Attachment | None:
        ...
        if not byte_content:
            __logger.warning(f"No thumbnail found for AAS with ID '{aas_identifier}' on server.")
            return None
    ...
    def get_submodel_by_id(self, submodel_identifier: str, level: Level = Level.default, extent: Extent = Extent.default) -> model.Submodel | None:
        ...
        if not self._client.submodels:
            __logger.error("Submodel API is not initialized in the client. Call 'initialize()' method of the client before calling this method.")
            return None
    ...
    def experimental_get_file_by_path_submodel_repo(self, submodel_identifier: str, id_short_path: str) -> Attachment | None:
        ...
        if not self._client.experimental:
            __logger.error("Experimental API is not initialized in the client. Call 'initialize()' method of the client before calling this method.")
            return None
        ...
        if not sme or not isinstance(sme, model.File):
            __logger.warning(f"No submodel element found at path '{id_short_path}' in submodel '{submodel_identifier}' on server.")
            return None

        byte_content = self._client.experimental.get_file_by_path_submodel_repo(submodel_identifier, id_short_path)

        if not byte_content:
            __logger.warning(f"No file found at path '{id_short_path}' in submodel '{submodel_identifier}' on server.")
            return None
...
def create_wrapper_by_dict(
    ...
    :return: An instance of SdkWrapper initialized with the provided parameters or None if initialization fails
    """
    __logger.info("Create AAS server wrapper from dictionary.")
...
def create_wrapper_by_config(
    ...
    :return: An instance of SdkWrapper initialized with the provided parameters or None if initialization fails
    """
    __logger.info(f"Create AAS wrapper client from configuration file '{config_file}'.")
    if not config_file.exists():
        config_string = "{}"
        __logger.warning(f"Configuration file '{config_file}' not found. Using default config.")
    else:
